...
{else}
    ...
    <form action="{$link->getModuleLink('swipp', 'validation', [], true)|escape:'html'}" method="post">
        <p>
            <img src="{$this_path_bw}swipp_wb.jpg" alt="{l s='Swipp' mod='swipp'}" width="86" height="49" style="float:left; margin: 0px 10px 5px 0px;" />
        ...
        <p>
            ...
            {if $currencies|@count > 1}
                {l s='We allow several currencies to be sent via swipp transfer.' mod='swipp'}
                <br /><br />
            ...
            {else}
                {l s='We accept the following currency to be sent by swipp mobile transfer:' mod='swipp'}&nbsp;<b>{$currencies.0.name}</b>
